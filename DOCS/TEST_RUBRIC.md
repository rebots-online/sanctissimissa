...
desktop/Android smoke observations are recorded.

| ID | Direct action and expected observation |
|---|---|
| AM-UI | On web, drop two real sample files (one photo, one short video) into `content/`, rebuild, open Help · About: both mount in the Origin Story at regular intervals, first figure floats RIGHT with text reflowed around it, second LEFT; captions derived from filenames; hover (desktop pointer) lifts/zooms the figure in place; click and short-tap open the lightbox (zoom-in entrance, ‹ › navigate with wrap, ✕/Esc/scrim close, double-tap toggles 2× zoom with pan; the video plays with controls). Remove the samples, rebuild: the page is identical to the pre-AM rendering (no placeholders, no layout residue). Version & Build shows the derivative-corpus Corpus line; the string "vendored, re-realized" appears nowhere in `src/`. At <720px width floats persist at reduced width with text still reflowed. On the Linux desktop build and the Android emulator build, media renders and lightbox tap works with no CSP console errors. |
