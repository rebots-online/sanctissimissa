...
| ＋ `updateAnnotation` | fn | `src/core/annotations/store.ts` | P-V | edit note/color of an existing annotation | `(id: string, patch: Partial<Pick<Annotation,'note'\|'color'>>) => void` |
...
| ＋ `AnnotationIndex` | comp | `src/ui/AnnotationIndex.tsx:1` | P-V | global annotation/highlight screen: `allAnnotations()` grouped by `nodeKey` (titles via `db`), edit (`updateAnnotation`) / color / remove / jump-to (`onOpenKey`); honest-absence empty state | `(props: { db; onOpenKey }) => JSX` |
...
| Δ `View` / `NAV` | type/const | `src/App.tsx:27` / `:29` | P-V | rail gains `annotations` (🔖) and the search entry | `type View` |
