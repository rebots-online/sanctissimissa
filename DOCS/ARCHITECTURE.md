...
| `ThemeModePreference` / `ThemePreference` / `ThemeSettingsStore` / `normalizeThemePreference` / `readThemePreference` / `writeThemePreference` | types/fns | `src/core/theme/themes.ts:17` | GL.1 | Validated appearance preferences, legacy migration, shared sidecar/local cache persistence | Exact fields/signatures in [optional-glass entity table](ARCHITECTURE/standroidsmissal-v1.39.15371-optional-glass-20260913.md) |
| Optional glass CSS / Glass surface set / Checkbox CSS / App theme restoration | rules/effect | `src/styles.css:953`, `src/App.tsx:85` | GL.1 / GL.2 | Palette-independent frosted surfaces; native labelled checkbox; restore at startup; opaque accessibility/print fallback | [Contract and design](ARCHITECTURE/standroidsmissal-v1.39.15371-optional-glass-20260913.md) |
