...
| Entity | Type | File:line | St | Role | Key signatures / fields |
|---|---|---|---|---|---|
| `ThemeFamily` / `ThemeMode` | types | `src/core/theme/themes.ts:1` | P-E | `'glass-acrylic'\|'glass-clear'\|'skeuomorphic'\|'retro-futurist'\|'brutalist'\|'retro-terminal'\|'sanctissimissa'\|'hello-word-glow'` (§7.7, DOCS/THEMES.md); `'light'\|'dark'`; `LEGACY_FAMILY_ALIASES`/`normalizeFamily` migrate renamed ids | — |
