...
| `InferenceBackend` / `InferenceSession` / `InferenceRouter` / `SelectionReport` | capability-probed adapters for `atomic-llama-cpp-turboquant` native (desktop; Android NDK where supported), WebGPU TurboQuant (`VENDORED/turboquant-wasm` engine; TQ-compressed-KV attention), `turboquant-wasm` WASM-SIMD fallback, hosted OpenAI-compatible services (later tier), and honest offline fallback; provider claims follow a real `probe()` + smoke generation; wllama dropped (operator decision 2026-09-07) |
