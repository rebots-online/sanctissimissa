...
- **Remotes (CC13):** `origin` = `$FORGEJO_URL` (see `~/Admin-Manual` for value) — was authoritative LFS store. `github` = old code-only mirror. `sanctissimissa` = `https://github.com/rebots-online/sanctissimissa.git` — active checkpoint remote. Old remotes preserved but not actively pushed (Forgejo server down as of 2026-09-04). LFS objects (30GB) remain in the original repo at `/home/robin/github/StAndroidsMissal/.git/lfs/`.
